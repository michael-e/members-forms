...
<xsl:stylesheet version="1.0"
  ...
  <!-- Configuration: Load the (language) configuration file and specify the class which should be added to invalid form elements. -->
  ...
  <xsl:template match="@message" mode="members:render-field-message">
    ...
    <xsl:param name="message">
      <!-- Find message with the correct node name. The placement inside the config file (which field?) must not matter here! (This placement only decides rendering of "invalid" classes for fields, see 'members:input' template.) -->
